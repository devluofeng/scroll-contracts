...
contract('PublicResolver', function (accounts) {
    ...
    beforeEach(async () => {
        node = namehash.hash('scroll');
        ens = await ENS.new();
        nameWrapper = await NameWrapper.new();
        resolver = await PublicResolver.new(ens.address, nameWrapper.address);
        await ens.setSubnodeOwner('0x0', sha3('scroll'), accounts[0], {from: accounts[0]});
    });
    ...
    describe('dns', async () => {
        ...
        it('permits setting name by owner', async () => {
            // a.scroll. 3600 IN A 1.2.3.4
            const arec = '016103657468000001000100000e10000401020304';
            // b.scroll. 3600 IN A 2.3.4.5
            const b1rec = '016203657468000001000100000e10000402030405';
            // b.scroll. 3600 IN A 3.4.5.6
            const b2rec = '016203657468000001000100000e10000403040506';
            // scroll. 86400 IN SOA ns1.scrolldns.xyz. hostmaster.test.scroll. 2018061501 15620 1800 1814400 14400
            ...
            assert.equal(await resolver.dnsRecord(node, sha3(dnsName('a.scroll.')), 1), '0x016103657468000001000100000e10000401020304');
            assert.equal(await resolver.dnsRecord(node, sha3(dnsName('b.scroll.')), 1), '0x016203657468000001000100000e10000402030405016203657468000001000100000e10000403040506');
            assert.equal(await resolver.dnsRecord(node, sha3(dnsName('scroll.')), 6), '0x03657468000006000100015180003a036e733106657468646e730378797a000a686f73746d6173746572057465737431036574680078492cbd00003d0400000708001baf8000003840');
        });

        it('should update existing records', async () => {
            // a.scroll. 3600 IN A 4.5.6.7
            const arec = '016103657468000001000100000e10000404050607';
            // scroll. 86400 IN SOA ns1.scrolldns.xyz. hostmaster.test.scroll. 2018061502 15620 1800 1814400 14400
            ...
            assert.equal(await resolver.dnsRecord(node, sha3(dnsName('a.scroll.')), 1), '0x016103657468000001000100000e10000404050607');
            assert.equal(await resolver.dnsRecord(node, sha3(dnsName('scroll.')), 6), '0x03657468000006000100015180003a036e733106657468646e730378797a000a686f73746d6173746572057465737431036574680078492cbe00003d0400000708001baf8000003840');
        })

        it('should keep track of entries', async () => {
            // c.scroll. 3600 IN A 1.2.3.4
            ...
            // Initial check
            var hasEntries = await resolver.hasDNSRecords(node, sha3(dnsName('c.scroll.')));
            assert.equal(hasEntries, true);
            hasEntries = await resolver.hasDNSRecords(node, sha3(dnsName('d.scroll.')));
            assert.equal(hasEntries, false);

            // Update with no new data makes no difference
            await resolver.setDNSRecords(node, rec, {from: accounts[0]});
            hasEntries = await resolver.hasDNSRecords(node, sha3(dnsName('c.scroll.')));
            assert.equal(hasEntries, true);
        ...
        it('can clear a zone', async () => {
            ...
            assert.equal(await resolver.dnsRecord(node, sha3(dnsName('a.scroll.')), 1), null);
        ...
        it('should handle single-record updates', async () => {
            // e.scroll. 3600 IN A 1.2.3.4
            const erec = '016503657468000001000100000e10000401020304';
            const rec = '0x' + erec;

            await resolver.setDNSRecords(node, rec, {from: accounts[0]});

            assert.equal(await resolver.dnsRecord(node, sha3(dnsName('e.scroll.')), 1), '0x016503657468000001000100000e10000401020304');
        })


        it('forbids setting DNS records by non-owners', async () => {
            // f.scroll. 3600 IN A 1.2.3.4
